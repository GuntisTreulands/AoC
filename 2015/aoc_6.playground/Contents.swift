...
func passInInstructions(_ string: String) -> Int
{
	var countOfTotalLightsTurnedOn = 0

	let size = 1000
	var table = [Bool](repeating: false, count: size * size)
	...
	print("total count \(instructions.count)")

	for instruction in instructions {

		index += 1

		var instructionType: Instruction
		var keyword: String
		...
		let splitInstructions = instruction.components(separatedBy: " through ")

		let instructionStartCoordinates = splitInstructions.first!.components(separatedBy: keyword).last!
		let instructionEndCoordinates = splitInstructions.last!

		let startInstructions = instructionStartCoordinates.components(separatedBy: ",")
		let endInstructions = instructionEndCoordinates.components(separatedBy: ",")

		let startX = Int(startInstructions.first!)!
		let startY = Int(startInstructions.last!)!

		let endX = Int(endInstructions.first!)!
		let endY = Int(endInstructions.last!)!

		for row in startX...endX {
			for column in startY...endY {
				let existingValue = table[row * size + column]

				switch instructionType {
					case .turn_on:
						table[row * size + column] = true
						if !existingValue {
							countOfTotalLightsTurnedOn += 1
						}
					case .turn_off:
						table[row * size + column] = false
						if existingValue {
							countOfTotalLightsTurnedOn -= 1
						}
					case .toggle:
						table[row * size + column] = !existingValue
						if existingValue {
							countOfTotalLightsTurnedOn -= 1
						} else {
							countOfTotalLightsTurnedOn += 1
	...
	print("countOfTotalLightsTurnedOn \(countOfTotalLightsTurnedOn)");

	return countOfTotalLightsTurnedOn
}


func passInInstructionsV2(_ string: String) -> Int
{
	var countOfTotalBrightness = 0
			...
			for column in startY...endY {
				...
				switch instructionType {
					case .turn_on:
						...
						countOfTotalBrightness += 1

					case .turn_off:
						table[row * size + column] = max(0, existingValue-1)
						if existingValue > 0 {
							countOfTotalBrightness -= 1
						}
					case .toggle:
						table[row * size + column] += 2
						countOfTotalBrightness += 2
				}
	...
	print("countOfTotalBrightness \(countOfTotalBrightness)");

	return countOfTotalBrightness
}
