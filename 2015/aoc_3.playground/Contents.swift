...
func countOfHousesThatReceiveAtLeastOnePresent(_ requirement: String) -> Int {
	...
	locations.insert(Location(x: currentXPosition, y: currentYPosition))

	for step in requirement {
		switch step {
			case ">":
				currentXPosition += 1
			case "<":
...
func roboCountOfHousesThatReceiveAtLeastOnePresent(_ requirement: String) -> Int {
	...
	var currentSantaXPosition: Int = 0
	var currentSantaYPosition: Int = 0

	var currentRoboXPosition: Int = 0
	var currentRoboYPosition: Int = 0

	locations.insert(Location(x: currentSantaXPosition, y: currentSantaYPosition))


	for (index, step) in requirement.enumerated() {
		switch step {
			case ">":
				if index % 2 == 0 { currentSantaXPosition += 1 } else { currentRoboXPosition += 1 }
			case "<":
				if index % 2 == 0 { currentSantaXPosition -= 1 } else { currentRoboXPosition -= 1 }
			case "^":
				if index % 2 == 0 { currentSantaYPosition -= 1 } else { currentRoboYPosition -= 1 }
			case "v":
				if index % 2 == 0 { currentSantaYPosition += 1 } else { currentRoboYPosition += 1 }
			default:
				continue
		...
		if index % 2 == 0 {
			...
			locations.insert(Location(x: currentRoboXPosition, y: currentRoboYPosition))
		}
	}
